...
app.post('/api/create/event', (req, res) => {
    try {
        ...
        console.log('JIIJAA' + emailUser)
        connection.query('SELECT user.id FROM user WHERE email = ?', [emailUser], (err, result, fields) => {
            console.log('JIIJAA' + result.id)
            if (err) throw err
            ...
            connection.query('SELECT id FROM `Group` WHERE id = ?', [idGroups], (err, results) => {
                if (err) throw err
                if (results.length != 0) {
                    connection.query(sqlCreateEvent, [idGroups,idUser, EventName, EventStartDate, EventEndDate, EventLocation, EventInfo], (err, results) => {
                            if (err) { throw err }
                            console.log(results)
...
app.post('/api/create/group', (req, res) => {
    try {
        ...
        console.log(req.body.email + '  SPOSTI')
        ...
        //TODO!! ADD RANDOM PASSWORD GENERATE AND CREATE GROUP
        connection.query(sqlNewGroup, [groupName, groupPassword], (err, results, fields) => {
            console.log('group id on '+results[0][0].id)
            if (err) throw err
            groupId = results[0][0].id
            

            connection.query('SELECT id FROM user WHERE email = ?', [userEmail], (err, result, fields) => {
                console.log('TEsti  ' + Object.keys(result[0]))
                if (err) throw err
                userId = result[0].id
                console.log(userId)
                connection.query(sqlAddToGroup, [groupId, userId], (err, results, fields) => {
                    console.log('TEsti123' + results)
                    if (err) throw err
                    res.status(200).json({
                        "CreateGroupResponse": [{
...
app.post('/api/user/groups', (req, res) => {
    try {
        ...
        connection.query('SELECT id FROM User WHERE email = ?', [emailUser], (err, result, fields) => {
...
app.post('/api/user/events', async (req, res) => {
        ...
        connection.query('SELECT id FROM User WHERE email = ?', [emailUser], (err, result, fields) => {
            ...
            if (result.length > 0) {
                connection.query(sqlGetUserEvents, [idUser], (err, results, fields) => {
                    console.log('EKA TESTI' + Object.keys(results[1].message))
                    if (err) throw err
                    if (results.length > 0) {
    ...
    const getUserEvents = (sentResults, sentIdUser, callback) => {
        ...
        async.forEachOf(sentResults[0], function (value, key, callback) {
            let eventId = sentResults[0][key].id
            console.log('EVENTID = ' + Object.keys(sentResults[0]))
            connection.query('SELECT groupName FROM `Group` WHERE id = (SELECT group_id FROM Event WHERE id = ?)',
                [eventId], (err, result, fields) => {
                    ...
                    connection.query(sqlGetInParticipants, [eventId], (err, result) => {
                        console.log(result[0] + ' TESI ' + result[0][0].numberOfParticipants)
                        if (err) throw err
                        ...
                        connection.query(sqlGetOutParticipants, [eventId], (err, result) => {
                            if (err) throw err
                            responseJSON[key]["numberOfParticipantsOut"] = result[0][0].numberOfParticipantsOut

                            connection.query('SELECT Entrant FROM Participant WHERE user_id = ? AND event_id = ?', [sentIdUser, eventId], (err, result) => {
                                if (err) throw err
...
app.post('/api/group/members', (req, res) => {
    try {
        ...
        connection.query(sqlGetGroupMembers, [groupId], (err, results, fields) => {
            if (err) throw err
            console.log('TESTETSTSTSTTS ' + results[0])
            if (results.length > 0) {
                res.status(200).json({
                    "groupMembers":
                        results[0]
...
app.post('/api/group/events', (req, res) => {
    ...
    connection.query('SELECT user.id FROM user WHERE email = ?', [userEmail], (err, result, fields) => {
        console.log(result)
        if (err) throw err
        let idUser = result[0].id
...
app.post('/api/group/addtogroup', (req, res) => {
    try {
        ...
        connection.query('SELECT id FROM User WHERE email = ?', [userEmail], (err, results) => {
            
            if (err) throw err
            if (results.length > 0) {
                const idUser = results[0].id
                connection.query('SELECT id FROM `Group` WHERE groupPassword = ?', [groupPassword], (err, results) => {
                    if (err) throw err
                    console.log(results)
                    console.log(results.length)
                    if (results.length > 0) {
                        const idGroup = results[0].id
                        connection.query(sqlGetUserGroups, [idUser], (err, results) => {
                            ...
                            for (let i = 0; i != results[0].length; i++) {
                                ...
                                if (results[0][i].idGroup == idGroup) {
...
app.post('/api/group/events/signtoevent', (req, res) => {
    try {
        ...
        connection.query('SELECT id FROM User WHERE email = ?', [userEmail], (err, results, fields) => {
            if (err) throw err
            const idUser = results[0].id
